...
async function summarizeWithOpenAI(items) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: "system", content: "Return JSON only. Summarize each crypto headline into one concise Korean sentence." },
        { role: "user", content: `JSON 형식: {"summaries":["...", "..."]}\n\n${promptItems}` }
      ]
    })
  });
...
export async function handler() {
  try {
    ...
    return { statusCode: 200, headers: { "content-type": "application/json; charset=utf-8" }, body: JSON.stringify(news) };
  } catch (err) {
    return { statusCode: 200, headers: { "content-type": "application/json; charset=utf-8" }, body: JSON.stringify([{ tag:"시장", headline:"뉴스 데이터를 불러오지 못했습니다.", time:"방금", source:"System", url:"#"}]) };
  }
}
